...
2.bus事件总线,思想源于java，相当于公交车，Bus.$emit上车，Bus.$on下车接受数据
...
页面的组件通过Dispatch 来调用Vuex的Actions 进行Actions逻辑处理数据，将处理好的数据通过commit 进行Mutations 中的操作,进行数据逻辑处理,并且修改state中的存储的数据，state 是响应式的，只要state中的数据发生修改，会重新渲染组件
...
vuex的State特性是？
答：
一、Vuex就是一个仓库，仓库里面放了很多对象。涉及多个组件储存的公共数据
...
computed（计算属性）有缓存
watch时刻监听data中的数据，无缓存，一旦在跟组件中写watch，则一直时刻监听，浪费性能
...
当读取某个对象的某个属性时都会执行一次搜索,首先从实例中找,如果没有找到的话,就几许去原型的原型上去找,就这样一直找到最顶成的Object.prototype,还是找不到,就会返回一个undefined

**constructor**:属性执行创建实例对象的构造函数

### 12.箭头函数this指向与普通函数的区别

箭头函数中的this是定义所在位置的对象
...
5.当一个文件里出现n多个 export 导出很多模块，导入时除了一个一个导入，也可以用import * as example
      import和require的区别：import和require都是被模块化使用
...
```
hash地址栏 URL 中的 # 符号,而history是没有#的。vue默认使用hash。 虽然出现在 URL 中，但不会被包括在 HTTP 请求中，对后端完全没有影响，因此改变 hash 不会重新加载页面。
...
通过history api，我们丢掉了丑陋的#，但是它也有个问题：不怕前进，不怕后退，就怕刷新，f5，因为刷新是实实在在地去请求服务器的。
（如果后端没有准备的话），则会刷新出来404页面。
```
...
​	1、查询DNS(域名解析),获取域名对应的 IP地址，查询浏览器缓存
​	2、浏览器与服务器建立TCP连接
​	3、浏览器向服务器发送http 请求
​	4、服务器接收到请求后，根据路径参数，经过后端的一些处理生成html代码 返回浏览器
...
### 31、前端工程化思想	（https://www.jianshu.com/p/88ed70476adb）
